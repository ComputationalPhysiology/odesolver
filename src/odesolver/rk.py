...
def rk_step(fun, t, y, f, h, A, B, C, K):
    ...
    for s, (a, c) in enumerate(zip(A[1:], C[1:]), start=1):
        ...
        K[s] = fun(t + c * h, y + dy.T)

    y_new = y + h * np.dot(K[:-1].T, B).T
    f_new = fun(t + h, y_new)
...
class RungeKutta(ODESolver):
    ...
    def __init__(
        ...
        **extraneous,
        ...
        self.rtol, self.atol = validate_tol(
            rtol=rtol,
            atol=atol,
        ...
        self.K = np.empty(
            (self.n_stages + 1, self.num_points, self.num_states), dtype=self.y.dtype
        )
        self.error_exponent = -1 / (self.error_estimator_order + 1)
        self.h_previous = None

    def _estimate_error(self, K, h):
        return (np.dot(K.T, self.E) * h).T
